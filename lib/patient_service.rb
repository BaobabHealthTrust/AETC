  ...
  def self.create_remote_person(received_params)
    ...
    known_demographics = {
      ...
      "patient"=>{
       ...
       "addresses"=>
            {"state_province"=> received_params["filter_district"],
            "address2"=> new_params["addresses"]["address2"],
            "address1"=> new_params["addresses"]["address1"],
            "neighborhood_cell"=> "",
            "city_village"=> received_params["filter"]["t_a"],
    ...
    begin
      ...
      person["person"]["addresses"]["state_province"] = received_params["filter_district"]
      person["person"]["addresses"]["address2"] = new_params["addresses"]["address2"]
      person["person"]["addresses"]["address1"] = new_params["addresses"]["address1"]
      person["person"]["addresses"]["neighborhood_cell"] = ""
      person["person"]["addresses"]["city_village"] = received_params["filter"]["t_a"]
      person["person"]["addresses"]["county_district"] = new_params["addresses"]["county_district"]
...
EOF
  ...
  def self.person_search(params)
    ...
    people = Person.find(:all, :limit => 50, :include => [{:names => [:person_name_code]}, :patient], :conditions => [
        "gender = ? AND \
