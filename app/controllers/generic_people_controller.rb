class GenericPeopleController < ApplicationController
  ...
  def search
        ...
        if create_from_remote
          found_person_name = PersonName.find_by_person_id(found_person.person_id)
          ...
          unless found_person_data.blank?
            if found_person_data['person']['names']['given_name'].upcase == found_person_name.attributes['given_name'].upcase and
              ...
              current_identifier =  current_national_id.identifier
              if found_person_data['person']['patient']['identifiers']['National id'].length == 6 and current_identifier.length != 6
